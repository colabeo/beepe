...
//    var MouseSync = InputSync.MouseSync;
    var TouchSync = require('custom/TouchSync');
    var MouseSync = require('custom/MouseSync');
    var Templates = require('custom/Templates');
    var Transform = require('famous/core/Transform');

    // import views
    var VerticalScrollView = require('custom/VerticalScrollView');
    var ContactItemView    = require('views/ContactItemView');
        ...
        function doResize() {
            ...
            if (this.node)
                var firstChar = _.chain(this.node.array)
                    .filter(function (i) {
                        return i instanceof ItemView
                    })
                    .map(function (i) {
                        return i.model.get(SortKey)[0] || '#'
                    }.bind(this))
                    .map(function (i) {
                        return (/^[a-zA-Z]+$/.test(i)) ? i.toUpperCase() : '#'
                    })
                    .value();
            var lastGroupLength = _.filter(firstChar, function (i) {
                return i == _.last(firstChar)
            }).length;
            var itemView = _.find(this.node.array, function (i) {
                return i instanceof  ItemView
            });
            var itemHeight = (itemView) ? itemView.getSize()[1] : 0;
            var headerView = _.find(_.clone(this.node.array).reverse(), function (i) {
                return i instanceof  HeaderView
            });
